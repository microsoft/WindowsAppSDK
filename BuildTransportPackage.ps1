...
# Oddly, the build scripts restors the solutions here and on each call to msbuild.
if(-not (test-path ".nuget\nuget.exe"))
{
...
# init MRT
 ...
 foreach($configurationToRun in $configuration.Split(","))
 {
    foreach($platformToRun in $platform.Split(","))
    {
        ...
        msbuild /restore "$MRTSourcesDirectory\mrt\MrtCore.sln" /p:Configuration=$configurationToRun,Platform=$platformToRun
    }
 }

$configurationToUseForMRTBinaries = "release"

# Prefer release, but use debug if avalible
foreach($configurationToUse in $configuration.split(","))
{
    if($configurationToUse -eq "debug")
    {
        $configurationToUseForMRTBinaries = "debug"
    }
}
...
foreach($platformToRun in $platform.Split(","))
{
    copy-item -path "$MRTBinariesDirectory\$configurationToUseForMRTBinaries\$platformToRun\mrm\mrm.dll" -destination "$fullNugetPath\runtimes\win10-$platformToRun\native" -force
    copy-item -path "$MRTBinariesDirectory\$configurationToUseForMRTBinaries\$platformToRun\mrm\mrm.pdb" -destination "$fullNugetPath\runtimes\win10-$platformToRun\native" -force
    copy-item -path "$MRTBinariesDirectory\$configurationToUseForMRTBinaries\$platformToRun\Microsoft.Windows.ApplicationModel.Resources\Microsoft.Windows.ApplicationModel.Resources.pdb" -destination "$fullNugetPath\runtimes\win10-$platformToRun\native" -force
    copy-item -path "$MRTBinariesDirectory\$configurationToUseForMRTBinaries\$platformToRun\Microsoft.Windows.ApplicationModel.Resources\Microsoft.Windows.ApplicationModel.Resources.dll" -destination "$fullNugetPath\runtimes\win10-$platformToRun\native" -force
    
    copy-item -path "$MRTBinariesDirectory\$configurationToUseForMRTBinaries\$platformToRun\mrm\mrm.lib" -destination "$fullNugetPath\lib\win10-$platformToRun" -force

    if($platformToRun -eq "x86")
    {
        copy-item -path "$MRTBinariesDirectory\$configurationToUseForMRTBinaries\$platformToRun\Microsoft.Windows.ApplicationModel.Resources.Projection\Microsoft.Windows.ApplicationModel.Resources.Projection.dll" -destination "$fullNugetPath\lib\net6.0-windows10.0.17763.0" -force
        copy-item -path "$MRTBinariesDirectory\$configurationToUseForMRTBinaries\$platformToRun\Microsoft.Windows.ApplicationModel.Resources.Projection\Microsoft.Windows.ApplicationModel.Resources.Projection.pdb" -destination "$fullNugetPath\lib\net6.0-windows10.0.17763.0" -force
        copy-item -path "$MRTBinariesDirectory\$configurationToUseForMRTBinaries\$platformToRun\Microsoft.Windows.ApplicationModel.Resources\Microsoft.Windows.ApplicationModel.Resources.winmd" -destination "$fullNugetPath\lib\uap10.0" -force
    }
}
...
# build AnyCPU
msbuild /restore "dev\Bootstrap\CS\Microsoft.WindowsAppRuntime.Bootstrap.Net\Microsoft.WindowsAppRuntime.Bootstrap.Net.csproj" /p:Configuration=$configurationToUseForMRTBinaries,Platform=anycpu

# If AnyCPU generates another dll it needs to be added here.
copy-item -path "buildoutput\$configurationToUseForMRTBinaries\anycpu\Microsoft.WindowsAppRuntime.Bootstrap.Net\Microsoft.WindowsAppRuntime.Bootstrap.Net.dll"  -destination "$fullNugetPath\lib\net5.0-windows10.0.17763.0"
