...
#define WINDOWSAPPRUNTIME_TEST_MSIX_DEPLOYMENT_MLFRAMEWORK_PACKAGE_NAME L"MicrosoftCorporationII.WinAppRuntime.MLFramework-T"
...
namespace DeploymentWindowsAppRuntimeMLFramework
{
    constexpr PCWSTR c_PackageDirName = L"Deployment.WindowsAppRuntime.Test.MLFramework";
    constexpr PCWSTR c_PackageFamilyName = WINDOWSAPPRUNTIME_TEST_MSIX_DEPLOYMENT_MLFRAMEWORK_PACKAGE_NAME L"_" WINDOWSAPPRUNTIME_TEST_MSIX_PUBLISHERID;
    constexpr PCWSTR c_PackageFullName = WINDOWSAPPRUNTIME_TEST_MSIX_DEPLOYMENT_MLFRAMEWORK_PACKAGE_NAME L"_" WINDOWSAPPRUNTIME_TEST_METADATA_VERSION_STRING L"_neutral__" WINDOWSAPPRUNTIME_TEST_MSIX_PUBLISHERID;
}
...
inline void AddPackage_DeploymentWindowsAppRuntimeMLFramework()
{
    AddPackage(Test::Packages::DeploymentWindowsAppRuntimeMLFramework::c_PackageDirName, Test::Packages::DeploymentWindowsAppRuntimeMLFramework::c_PackageFullName);
}

inline void RemovePackage_DeploymentWindowsAppRuntimeMLFramework()
{
    // Best-effort removal. PackageManager.RemovePackage errors if the package
    // is not registered, but if it's not registered we're good. "'Tis the destination
    // that matters, not the journey" so regardless how much or little work
    ...
    //
    // Thus, do a *IfNecessary removal
    RemovePackageIfNecessary(Test::Packages::DeploymentWindowsAppRuntimeMLFramework::c_PackageFullName);
}

inline bool IsPackageRegistered_DeploymentWindowsAppRuntimeMLFramework()
{
    return IsPackageRegistered(Test::Packages::DeploymentWindowsAppRuntimeMLFramework::c_PackageFullName);
}
