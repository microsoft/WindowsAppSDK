...
namespace Test::CompatibilityTests
{
    ...
    //
    // NOTE: the wrapper is named CallContainmentTestInitialize (not
    // ContainmentTestInitialize) so MSVC's linker does not pull in the
    // Microsoft.WindowsAppRuntime.lib import stub for the same name -- doing so
    // would introduce a static dependency on Microsoft.WindowsAppRuntime.dll,
    // load a second copy in the test folder, and break the catalog swap.
    static HRESULT CallContainmentTestInitialize(bool enableTest) noexcept
    {
    ...
    public:
        ...
        TEST_METHOD_SETUP(MethodInit)
        {
            ...
            VERIFY_ARE_EQUAL(S_OK, CallContainmentTestInitialize(true));
            return true;
        }
        ...
        TEST_METHOD_CLEANUP(MethodUninit)
        {
            ...
            VERIFY_ARE_EQUAL(S_OK, CallContainmentTestInitialize(false));
