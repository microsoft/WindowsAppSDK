...
- [4. Examples](#4-examples)
  ...
  - [4.6. PackageVolume.RepairPackageVolume()](#46-packagevolumerepairpackagevolume)
- [5. Remarks](#5-remarks)
...
It's possible this tracking information can become invalidate. For example, backup a drive's
content, replace the drive with a new one and then restore the content. The packages installed on
this new drive's PackageVolume(s) aren't recognized by Windows because the new drive has a different
media ID.

The Windows App SDK's new `PackageVolume.RepairPackageVolume()` detects and corrects these conditions.

`PackageVolume.CheckPackageVolume()` checks if the PackageVolume is OK or in need of repair.

**NOTE:** `RepairPackageVolume()` requires admin privilege (`CheckPackageVolume()` doesn't).
...
## 4.6. PackageVolume.RepairPackageVolume()
...
var CheckAndFixPackageVolume(string path)
{
    ...
    if (!packageVolume.CheckPackageVolume())
    {
        bool ok = PromptUserForConfirmation();
        if (ok)
        {
            try
            {
                packageVolume.RepairPackageVolume();
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.ToString());
...
namespace Microsoft.Windows.Management.Deployment
{
    ...
    runtimeclass PackageVolume
    {
        ...
        bool CheckPackageVolume();

        /// Repair the package volume (if necessary).
        void RepairPackageVolume();
    };
