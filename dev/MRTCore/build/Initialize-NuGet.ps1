...
$nuget_exe = . "$PSScriptRoot\Initialize-DownloadLatest.ps1" -OutDir $toolsDir -DownloadUrl "https://dist.nuget.org/win-x86-commandline/v5.8.0-preview.3/nuget.exe" -DownloadName $nugetDownloadName -Unzip $false

Write-Progress "Downloading the AzureDevOps CredentialProvider"
...
. "$PSScriptRoot\Initialize-InstallFromNuget.ps1" -OutDir $toolsDir -DownloadFeed $credProviderDownloadFeed -PackageName $credProviderPackageName -targetFileName $credProviderExeName -Verbosity $Verbosity
