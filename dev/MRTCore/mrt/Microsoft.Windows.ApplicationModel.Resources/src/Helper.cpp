...
HRESULT GetDefaultPriFile(winrt::hstring& filePath)
{
    ...
    if (WinAppSdk::Containment::IsChangeEnabled<WINAPPSDK_CHANGEID_62382643>())
    {
        ...
        // unpackaged discovery which also searches for "[modulename].pri". MrmGetFilePathFromName
        // may now return S_OK with a best-effort (possibly non-existent) path, so gate the fallback
        // on whether the packaged lookup actually resolved to an existing file rather than solely on
        // IsResourceNotFound(hr).
        if (isPackaged && (IsResourceNotFound(hr) || (SUCCEEDED(hr) && FAILED(CheckFile(filePath.c_str())))))
        {
            ...
            // Only adopt the unpackaged result if it resolves to an existing file; otherwise keep
            // the packaged best-effort path so ResourceManager still receives a path (and handles
