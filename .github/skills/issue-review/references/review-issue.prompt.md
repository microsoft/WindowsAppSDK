...
- `github_issue_attachments`: use when the issue/PR mentions attached ZIPs (DiagnosticsReport_*.zip, logs.zip, debug.zip) or asks to analyze logs/diagnostics. Always provide `extractFolder` as `Generated Files/issueReview/{{issue_number}}/logs`
...
- Suggest additional information needed: screenshots, logs, environment details, OS version, SDK/product version, error messages
...
- Suggest 2-3 potential assignees with context: `@<username> - <reason>` (e.g., "fixed similar rendering bug in #12345", "maintains AppLifecycle module")
