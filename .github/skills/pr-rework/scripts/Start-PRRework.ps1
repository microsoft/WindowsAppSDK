...
$newWorktreeScript = Join-Path $repoRoot '.github/skills/worktree-manager/scripts/New-WorktreeFromBranch.ps1'
...
function Get-OrCreateWorktree {
    ...
    . (Join-Path $repoRoot '.github/skills/worktree-manager/scripts/WorktreeLib.ps1')
...
After fixing, build the changed projects using BuildAll.ps1 scoped to the changed directories.
...
function Invoke-BuildPhase {
    ...
    try {
        ...
        $buildScript = Join-Path $State.worktreePath 'BuildAll.ps1'
        if (-not (Test-Path $buildScript)) {
            $buildScript = Join-Path $repoRoot 'BuildAll.ps1'
        }
...
function Invoke-TestPhase {
    ...
    try {
        ...
        foreach ($testProj in $testProjects) {
            ...
            $buildScript = Join-Path $State.worktreePath 'BuildAll.ps1'
            if (-not (Test-Path $buildScript)) { $buildScript = Join-Path $repoRoot 'BuildAll.ps1' }
...
try {
    ...
    if (-not $essentialsDone) {
        ...
        $buildEssentials = Join-Path $state.worktreePath '.github/skills/build-repo/scripts/Setup-BuildPrerequisites.ps1'
        if (-not (Test-Path $buildEssentials)) {
            $buildEssentials = Join-Path $repoRoot '.github/skills/build-repo/scripts/Setup-BuildPrerequisites.ps1'
        }
