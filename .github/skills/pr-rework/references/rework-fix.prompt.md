...
2. If specific projects changed, build them: `BuildAll.ps1 -Path <project-dir>`
...
1. Find test projects related to the changed code:
   ...
   - Match by product code prefix (e.g., changes to `AppLifecycle` → look for `AppLifecycleUnitTests`).
...
## Quality Rules
- Follow existing code style (`.editorconfig`, `.editorconfig`, .editorconfig).
