...
- `.github/skills/worktree-manager/scripts/WorktreeLib.ps1` available (for worktree management)
...
```
Phase 0 — BUILD ESSENTIALS: One-time .github/skills/build-repo/scripts/Setup-BuildPrerequisites.ps1 (NuGet restore)
...
for each iteration (1..MaxIterations):
  ...
  Phase 5 — BUILD:   Run BuildAll.ps1 for ALL changed projects
...
1. **Builds** the changed projects:
   ...
   - Runs `BuildAll.ps1` scoped to those projects
   - Checks exit code 0
