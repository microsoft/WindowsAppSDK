...
.PARAMETER RepositoryName
    The GitHub repository name. Auto-detected from git remote if not specified.
...
# Auto-detect repository owner/name if not specified
if (-not $RepositoryName -or -not $RepositoryOwner) {
    $slug = gh repo view --json nameWithOwner --jq .nameWithOwner 2>$null
    if ($slug -and $slug -match '/') {
        $parts = $slug -split '/'
        if (-not $RepositoryOwner) { $RepositoryOwner = $parts[0] }
        if (-not $RepositoryName) { $RepositoryName = $parts[1] }
    }
}
