...
function Post-PRReviewComment {
    ...
    $json | gh api "repos/{{OWNER}}/{{REPO}}/pulls/$PRNumber/reviews" --method POST --input - 2>&1
}
...
function Post-PRComment {
    ...
    $payload | gh api "repos/{{OWNER}}/{{REPO}}/issues/$PRNumber/comments" --method POST --input - 2>&1
}
