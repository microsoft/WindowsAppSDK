...
```cpp
// GOOD: Documents WinRT threading requirement
// This method must be called on the STA thread that owns the object.
// Cross-apartment calls require agile references.
void OnActivated() { ... }

// GOOD: Documents WIL macro usage
// THROW_IF_FAILED propagates the HRESULT as a C++ exception.
// Use RETURN_IF_FAILED in functions that return HRESULT instead.
THROW_IF_FAILED(m_factory->CreateInstance(outer, &inner));

// GOOD: IDL doc comment for public API
/// [doc_string("Retrieves the deployment status for the current package.")]
/// [doc_default_value("")]
