...
You are evaluating a pull request for the current repository.
...
1. **Fetch images and attachments** from PR #{{PR_NUMBER}} in `{{OWNER}}/{{REPO}}`:
   - Use `github_issue_images` tool with owner=`microsoft`, repo=`{{REPO}}`, issueNumber={{PR_NUMBER}}
   - Use `github_issue_attachments` tool with owner=`microsoft`, repo=`{{REPO}}`, issueNumber={{PR_NUMBER}}, extractFolder=`{{EXTRACT_FOLDER}}`

2. **Read the full discussion** using:
   ```
   gh pr view {{PR_NUMBER}} --repo $Repository --json body,comments,reviews,reviewRequests
   ```
