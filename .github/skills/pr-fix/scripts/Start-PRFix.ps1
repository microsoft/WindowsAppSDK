...
function Get-ActiveComments {
    ...
    try {
        $comments = gh api "repos/{{OWNER}}/{{REPO}}/pulls/$PRNumber/comments" 2>$null | ConvertFrom-Json
...
function Get-UnresolvedThreadCount {
    ...
    try {
        $result = gh api graphql -f query="query { repository(owner: `"$Owner`", name: `"$Repo`") { pullRequest(number: $PRNumber) { reviewThreads(first: 100) { nodes { isResolved } } } } }" 2>$null | ConvertFrom-Json
