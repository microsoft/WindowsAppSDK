...
$ProductKeywords = @{
    "Product-AppLifecycle" = @("fancy zones", "applifecycle", "zone", "snap", "layout", "window arrangement", "virtual desktop")
    "Product-SDK component" = @("run", "launcher", "alt+space", "alt space", "search", "plugin", "sdk run")
    "Product-Color Picker" = @("color picker", "appnotifications", "eyedropper", "hex", "rgb", "color code")
    ...
    if (-not $category) {
        ...
        if ($body -and $body.Length -gt 0) {
            ...
            if ($body -notmatch '(?i)(version|v\d+\.\d+)') {
                $missingItems += "SDK/product version"
            }
        } else {
