...
@{
    ...
    ExecutionParameters = @{
        ...
        Command    = 'copilot'
        Arguments  = @('-p', 'Review PR #45601 in {{OWNER}}/{{REPO}}...', '--yolo')
        WorkingDir = 'C:\s\repo'
        OutputDir  = 'C:\s\repo\output\copilot\45601'
        LogPath    = 'C:\s\repo\output\copilot\45601\_copilot-review.log'
    }
    MonitorFiles = @('C:\s\repo\output\copilot\45601\_copilot-review.log')
    CleanupTask  = $null
}
```
...
@{
    ...
    ExecutionParameters = @{
        ...
        Command    = 'claude'
        Arguments  = @('-p', 'Review PR #45601 in {{OWNER}}/{{REPO}}...',
                       '--dangerously-skip-permissions',
                       '--debug', 'all', '--debug-file', 'C:\output\claude\45601\_claude-debug.log')
        WorkingDir = 'C:\s\repo'
        OutputDir  = 'C:\s\repo\output\claude\45601'
        LogPath    = 'C:\s\repo\output\claude\45601\_claude-review.log'
    }
    MonitorFiles = @('C:\s\repo\output\claude\45601\_claude-debug.log')
    CleanupTask  = {
        param($Tracker)
...
```powershell
@{
    Label               = 'lint-module-applifecycle'
    ExecutionParameters = @{
        JobName    = 'lint-applifecycle'
        Command    = 'dotnet'
        Arguments  = @('build', '--no-restore', '-warnaserror')
        WorkingDir = 'C:\s\repo\src\modules\applifecycle'
        OutputDir  = 'C:\s\repo\output\lint\applifecycle'
        LogPath    = 'C:\s\repo\output\lint\applifecycle\build.log'
    }
    MonitorFiles = @('C:\s\repo\output\lint\applifecycle\build.log')
    CleanupTask  = $null
}
```
