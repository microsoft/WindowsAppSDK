...
function Get-PRActiveCommentCount {
    ...
    try {
        ...
        $comments = gh api "repos/{{OWNER}}/{{REPO}}/pulls/$PRNumber/comments" --jq '[.[] | select(.in_reply_to_id == null)] | length' 2>$null
        if ($comments) {
            return [int]$comments
        }
        return 0
...
try {
    ...
    if ($Labels) {
        ...
        $labeledIssues = gh issue list --repo $Repository --label "$Labels" --state open --limit $Limit --json number 2>$null | ConvertFrom-Json
