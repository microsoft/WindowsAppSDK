...
$worktreeLib = Join-Path $repoRoot '.github/skills/worktree-manager/scripts/WorktreeLib.ps1'
...
function Get-PRStatus {
    ...
    try {
        $commentCount = gh api "repos/{{OWNER}}/{{REPO}}/pulls/$PRNumber/comments" --jq '[.[] | select(.in_reply_to_id == null)] | length' 2>$null
    ...
    try {
        $threads = gh api graphql -f query="query { repository(owner: `"$Owner`", name: `"$Repo`") { pullRequest(number: $PRNumber) { reviewThreads(first: 100) { nodes { isResolved } } } } }" --jq '.data.repository.pullRequest.reviewThreads.nodes | map(select(.isResolved == false)) | length' 2>$null
