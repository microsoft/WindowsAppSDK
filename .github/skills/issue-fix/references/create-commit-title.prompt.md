...
1. From the diff, find the dominant area (e.g., `src/modules/*`, `docs/**`) and the change type (bug fix, docs update, config tweak).
...
**Scope rules**
- Use a short, repo-focused scope (one word preferred). Common scopes:
  - Core: `runner`, `settings-ui`, `common`, `docs`, `build`, `ci`, `installer`, `gpo`, `dsc`
  - Modules: `applifecycle`, `powerrename`, `awake`, `appnotifications`, `imageresizer`, `keyboardmanager`, `mouseutils`, `peek`, `hosts`, `file-locksmith`, `screen-ruler`, `text-extractor`, `cropandlock`, `paste`, `powerlauncher`
...
**Examples**
- `feat(applifecycle): add canvas template duplication`
