...
applyTo: '**/azure-pipelines.yml, **/azure-pipelines*.yml, **/*.pipeline.yml, **/WindowsAppSDK-*.yml, **/ProjectReunion-*.yml'
---

# Azure DevOps Pipeline YAML Best Practices

Guidelines for creating maintainable, secure, and efficient Azure DevOps pipelines in WindowsAppSDK.
...
## WindowsAppSDK Specific Patterns

### SDK Pipeline Naming Conventions

Pipelines in the Windows App SDK repo follow specific naming patterns. See the `build/` directory for existing templates:

- `WindowsAppSDK-Foundation-*.yml` – Foundation build pipelines
- `WindowsAppSDK-*.yml` – General SDK pipelines
- `ProjectReunion-*.yml` – Legacy pipeline names (Project Reunion)

### Using Pipeline Templates

Leverage existing templates from `build/AzurePipelinesTemplates/` for consistent configuration:

```yaml
# Example: Using a template
stages:
- template: build/AzurePipelinesTemplates/WindowsAppSDK-BuildMachine-Setup-Steps.yml
  parameters:
    Platform: x64
...
1. **Triggers**: Configure pull request validation with path filters for your component directory
2. **Branch Filters**: Include `refs/heads/main`
3. **Path Filters**: Include relevant `dev/<ComponentName>/` paths
4. **Variables**: Use `build/WindowsAppSDK-CommonVariables.yml` for shared settings

### Build Configurations

SDK components should build for all supported configurations:
...
For PR validation, use the `TransportPackage-Foundation-PR` pipeline as referenced in `docs/pull_request_template.md`.
