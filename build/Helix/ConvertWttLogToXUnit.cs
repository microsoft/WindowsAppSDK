...
namespace HelixTestHelpers
{
    public static class TestResultParser
    {
        ...
        public static void ConvertWttLogToXUnitLog(string wttInputPath, string xunitOutputPath, string testNamePrefix, string helixworkitem)
        {
            var testPass = TestResultParser.ParseTestWttFile(wttInputPath, helixworkitem, true);
            ...
            assembly.SetAttributeValue("name", "DCPPHelix.dll");
        ...
        public static TestPass ParseTestWttFile(string fileName, string workItem, bool cleanupFailuresAreRegressions)
        {
            using (var stream = System.IO.File.OpenRead(fileName))
            {
                ...
                foreach (XElement element in doc.Root.Elements())
                {
                    ...
                    if (element.Name == "RTI")
                    {
                        ...
                        string processName = element.Attribute("ProcessName").Value;
                        jobId = processName.Split('\\')[3];
                    }
                    ...
                    if (currentResult != null && element.Name == "Msg")
                    {
                        ...
                        if (dataElement != null)
                        {
                            ...
                            if (supportingInfo != null)
                            {
                                ...
                                {
                                    var screenshotName = screenshot.Substring(screenshot.LastIndexOf('\\') + 1);
                                    currentResult.Details += "\r\n[Screenshot]: " + "https://helix.dot.net/api/2019-06-17/jobs/" + jobId + "/workitems/" + workItem + "/files/" + screenshotName;
                                }
                            }
                        }
                    }
